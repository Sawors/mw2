...
//Program//
void main(){
    float bloomFactor = min(isEyeInWater + rainStrength * pow(eBS, 4), 1.0);
		...
		vec3 blur = BloomTile(2.0, vec2(0.0      , 0.0   ));
			...
			blur += BloomTile(5.0, vec2(0.2075   , 0.26  )) * (0.8 + bloomFactor*0.2);
			blur += BloomTile(6.0, vec2(0.135    , 0.3325)) * (0.8 + bloomFactor*0.2);
			blur += BloomTile(7.0, vec2(0.160625 , 0.3325)) * (0.6 + bloomFactor*0.2);
			blur += BloomTile(8.0, vec2(0.1784375, 0.3325)) * (0.4 + bloomFactor*0.4);
			#endif
	#else
		vec3 blur = BloomTile(2.0, vec2(0.0      , 0.0   ));
			blur += BloomTile(3.0, vec2(0.0      , 0.26  ));
