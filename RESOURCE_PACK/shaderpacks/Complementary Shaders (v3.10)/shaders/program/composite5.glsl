...
void UnderwaterDistort(inout vec2 texCoord){
	...
	texCoord +=vec2(cos(texCoord.y * 32.0 + frameTimeCounter * 3.0),
	                sin(texCoord.x * 32.0 + frameTimeCounter * 1.7)) * 0.001 * UNDERWATER_DISTORT;
...
void Bloom(inout vec3 color, vec2 coord){
	...
	float bloomStrength = BLOOM_STRENGTH;
		...
		color = mix(color, blur, 0.18 * bloomStrength);
	#else
		color = mix(color, blur, 0.54 * bloomStrength);
	#endif
}
...
void main(){
	...
	vec3 RTX = vec3(0.0);
	for(int i = 0; i < 300; i++) {
		vec3 filmGrain = texture2D(noisetex, (1.0 + i * 0.1) * texCoord * vec2(viewWidth, viewHeight) / 512.0).rgb;
		RTX += (filmGrain - 0.25) / 128.0;
	}
	color += pow(RTX * 1.56, vec3(40.0));
	*/
