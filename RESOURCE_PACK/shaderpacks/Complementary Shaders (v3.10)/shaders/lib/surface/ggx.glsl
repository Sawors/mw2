...
float GGX(vec3 normal, vec3 viewPos, vec3 lightVec, float smoothness, float f0, float sunSize) {
    ...
    float roughnessX = roughness * roughness;
    ...
    specular = max(specular, 0.0) * (1.0 - roughnessX * (1.0 - 0.025 * f0));
...
float waterGGX(vec3 normal, vec3 oldNormal, vec3 nViewPos, vec3 lightVec, float smoothness, float f0, float sunSize) {
    float roughness = 1.0 - smoothness;
    roughness = 0.5;
    //if (sunVisibility == 0.0) roughness = 0.3;
    roughness *= roughness; roughness *= roughness;

    vec3 halfVec = normalize(lightVec - nViewPos);

    float dotLH = clamp(dot(halfVec, lightVec), 0.0, 1.0);
    float dotNewL = clamp(dot(normal,  lightVec), 0.0, 1.0);
    float dotNmOL = clamp(dot(normal - oldNormal,  lightVec), 0.0, 1.0);
    
    float dotNH = GetNoHSquared(sunSize, dotNewL, dot(normal, -nViewPos), dot(-nViewPos, lightVec));
    dotNmOL = min(dotNmOL * 40.0, 1.0);
    dotNH *= dotNH * dotNmOL;
    ...
    float specular = max(dotNewL * dotNewL * D * F / (dotLH * dotLH * (1.0 - k2) + k2), 0.0);
    specular = max(specular, 0.0) * (1.0 - roughness * (1.0 - 0.025 * f0));
    specular = specular / (0.125 * specular + 1.0);

    float timeBrightnessM = timeBrightness * timeBrightness;
    timeBrightnessM *= timeBrightnessM;
    timeBrightnessM *= timeBrightnessM;
    timeBrightnessM *= timeBrightnessM;

    if (sunVisibility == 0.0) specular *= 0.15;
    specular *= 0.1 + 0.9 * dotNmOL;

    //specular = smoothstep(0.0, 0.125, min(specular, 0.125));

    return max(specular * (20.0 + timeBrightnessM*50.0) * (1.0 - isEyeInWater*0.75), 0.0);
}
...
vec3 GetSpecularHighlight(float smoothness, float metalness, float f0, vec3 specularColor,
                          vec3 rawAlbedo, vec3 shadow, vec3 normal, vec3 viewPos, float materialFormat){
