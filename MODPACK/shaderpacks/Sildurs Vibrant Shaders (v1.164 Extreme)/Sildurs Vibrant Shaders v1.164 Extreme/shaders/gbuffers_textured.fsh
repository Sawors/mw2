...
vec4 worldposition = gbufferModelViewInverse * fragposition;
	...
	if (max(abs(worldposition.x),abs(worldposition.y)) < 0.99) {
		...
		diffuse *= dot(vec4(shadow2D(shadow,vec3(worldposition.st + vec2(halfres,halfres), worldposition.z)).x,
		shadow2D(shadow,vec3(worldposition.st + vec2(-halfres,halfres), worldposition.z)).x,
		shadow2D(shadow,vec3(worldposition.st + vec2(halfres,-halfres), worldposition.z)).x,
		shadow2D(shadow,vec3(worldposition.st + vec2(-halfres,-halfres), worldposition.z)).x),vec4(0.25));
	}
